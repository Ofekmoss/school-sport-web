define(["templates/manage", "manage/dal", "utils", "services/access", "components/multiselect-search", "components/selectex"], function (templates, dal, utils, Access) {
    ...
    function ReadAllData(comp, callback) {
        ...
        var promises = [dal.getRegions(), dal.getSports(), dal.getChampionships({season: comp.result.season})];
        ...
        utils.promiseAll(promises).then(function(results) {
            ...
            comp.sports = [];
            for (var i = 0; i < comp.championships.length; i++) {
                var championship = comp.championships[i];
                if (championship.Sport != null) {
                    var existingSport = comp.sports.find(function(s) {
                        return s.Id == championship.Sport.Id;
                    });
                    if (existingSport == null) {
                        comp.sports.push(championship.Sport);
                    }
                }
            }
            ...
            if (!comp.initFilters) {
                //console.log('no init filters');
                comp.result.team = null;
                comp.result.category = null;
                comp.result.championship = null;
                comp.result.sport = null;
                comp.result.region = null;
                comp.result.description = 'ניהול שחקנים';
                comp.load('clear');
                callback();
                return;
            }
    ...
    var FiltersPlayers = Vue.extend({
        ...
        methods: {
            seasonChanged: function() {
                var comp = this;
                //console.log('season changed: ' + comp.result.season.id + ', mounting? ' + comp.mounting);
                if (!comp.mounting) {
                    ...
                    if (season) {
                        ...
                        Vue.http.post(url, {season: season}).then(function (resp) {
                            //console.log('setting init filters to null');
            ...
            load: function(sender) {
                ...
                this.result.sender = sender;

                if (sender === 'click' || sender === 'init' || sender === 'clear') {
        ...
        watch: {
            'result.region': function(curr) {
                ...
                dal.getChampionships({season: comp.result.season, region: region}).then(function (championships) {
                ...
                /*
                utils.promiseAll([dal.getSports(region), dal.getChampionships({season: comp.result.season, region: region})]).then(function (results) {
                    comp.result.category = null;
                    comp.result.championship = null;
            ...
            'result.sport': function(curr) {
                ...
                utils.promiseAll([dal.getChampionships({season: comp.result.season, region: region, sport: sport})]).then(function (results) {
                    comp.result.category = null;
                    comp.result.championship = null;
