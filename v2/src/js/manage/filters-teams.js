define(["templates/manage", "manage/dal", "utils", "services/access", "components/multiselect-search", "components/selectex"], function (templates, dal, utils, Access) {
    ...
    function ReadAllData(comp, callback) {
        ...
        utils.promiseAll([dal.getRegions(), dal.getSports(), dal.getChampionships({season: comp.result.season})]).then(function(results) {
    ...
    var FiltersTeams = Vue.extend({
        ...
        watch: {
            ...
            'result.sport': function(curr) {
                var comp = this;
                var filters = {
                    sport: curr && curr.id ? curr.id : null,
                    region: this.result.region ? this.result.region.id : null,
                    season: this.result.season ? this.result.season.id : null
                };
            ...
            'result.region': function(curr) {
                ...
                var filters = {
                    ...
                    region: curr && curr.id ? curr.id : null,
                    season: this.result.season ? this.result.season.id : null
                };
                dal.getChampionships(filters).then(function (championships) {
                    comp.result.category = null;
                    comp.result.championship = null;
                    comp.championships = championships;
                    comp.sports = [];
