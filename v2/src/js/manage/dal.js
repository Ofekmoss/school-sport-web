define ([], function() {
    ...
    return {
        ...
        getChampionships: function(filters) {
            ...
            if (filters) {
                ...
                if (filters.season) {
                    if (query.length > 0)
                        query += '&';
                    var season =  filters.season.id ? filters.season.id : filters.season;
                    query += 'season=' + season;
                }
            }
        ...
        getPlayers: function(params) {
            ...
            var url = '/api/v2/manage/players?' + query.join('&');
            return Vue.http.get(url).then(function(res) {
        ...
        deletePlayer: function(playerId, teamId, studentId) {
            var url = '/api/v2/manage/players?';
            if (playerId)
                url += 'player=' + playerId + '&';
            if (teamId)
                url += 'team=' + teamId + '&';
