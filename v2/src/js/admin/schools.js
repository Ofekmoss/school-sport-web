define(["templates/admin", "utils", "dialog", "services/access", "consts", "components/multiselect", "generic/data-table"],
    function (templates, utils, Dialog, Access, consts) {
        ...
        function readSchools(comp, callback) {
            ...
            var url = '/api/v2/admin/schools?' + query;
            Vue.http.get(url).then(function (resp) {
                console.log('read ' + resp.body.length + ' schools from ' + url)
                comp.schools = [];
                for (var i = 0; i < resp.body.length; i++) {
                    ...
                    if (school.digitalSignatureDetails) {
                        if (school.digitalSignatureDetails.DateLastSigned != null)
                            school.digitalSignatureStatus = 'completed';
                        else if (school.digitalSignatureDetails.DateCreated != null)
                            school.digitalSignatureStatus = 'pending';
                    }
                    comp.schools.push(school);
                    /*
                    digitalSignatureDetails
                        DateCreated: "2023-07-31T10:30:00.000Z"
                        DateLastSigned: null
                        DocumentId: "45dc827e-474d-4be7-9649-08db913fcf9c"
                        PdfCaption: null
                        PdfFileName: "ClubRegistrationForm-1306-71.pdf"
                    */
                }
        ...
        var allColumns = [
            ...
            },
            {
                ...
                detailsField: 'digitalSignatureDetails',
                active: true,
                getSortValue: function(school) {
                    return school.digitalSignatureStatus || '';
                }
            },
            {
