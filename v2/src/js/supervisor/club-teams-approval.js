        ...
        function readTeams(comp) {
            ...
            Vue.http.get(url).then(function (resp) {
                for (var i = 0; i < resp.body.length; i++) {
                    ...
                    if (team.category) {
                        team.coordinatorApproval = team.clubDetailsConfirmed == null ? 'אין' : 'יש';
                        team.principalApproval = (team.approved & 2) > 0 ? 'יש' : 'אין'; //Teams.Status.PrincipalApproval
                        team.representativeApproval = (team.approved & 4) > 0 ? 'יש' : 'אין'; //Teams.Status.RepresentativeApproval
                        team.regionalManagerApproval = team.teamStatus == 2 ? 'יש' : 'אין';
            ...
            data: function () {
                return {
                    ...
                    columns: [
                        ...
                        {
                            name: 'אישור רכז',
                            key: 'coordinatorApproval',
                            active: true
                        },
                        {
                            name: 'אישור מנהל/ת בי"ס',
                            key: 'principalApproval',
                            active: true
                        },
                        {
                            name: 'אישור נציג',
                            key: 'representativeApproval',
                            active: true
                        },
                        {
                            name: 'אישור מנהל/ת מחוז',
                            key: 'regionalManagerApproval',
                            active: true
                        }],
                    sports: [],
                    regions: [],
