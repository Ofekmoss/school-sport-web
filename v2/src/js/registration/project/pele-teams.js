define(["templates/registration", "services/access", "dialog", "utils"], function (templates, Access, Dialog, utils) {
    ...
    function getTeams(comp) {
        Vue.http.get('/api/v2/registration/project/3/teams?players=1').then(function(result) {
            ...
            comp.teams.forEach(function(team, index) {
                ...
                var sportExist = false;
                for (var n = 0; !sportExist && n < comp.sports.length; n++) {
                    var sport = comp.sports[n];
                    sportExist = sport.name === team.sport.name && !!sport.team === !!team.sport.team;
                }
                if (!sportExist) {
                    comp.sports.push({name: team.sport.name, team: team.sport.team});
                }
            });
    ...
    function deleteTeams(teams) {
        return Vue.http.post('/api/v2/registration/project/3/teams/delete', teams);
    }
    ...
    var RegistrationProjectPeleTeamsComponent = Vue.extend({
        template: templates["pele-teams"],
        data: function () {
            return {
                user: Access.user,
                teams: [],
                ...
                days: ["א'", "ב'", "ג'", "ד'", "ה'", "ו'"],
                sports: [
                    {name: 'כדורגל', team: true},
                    {name: 'כדורסל', team: true},
                    {name: 'אתלטיקה'},
                    {name: 'טניס'},
                    {name: 'ג׳ודו'},
                    {name: 'האבקות'},
                    ...
                    {name: 'סיוף'},
                    {name: 'שחיה'},
                    {name: 'כדורעף', team: true},
                    {name: 'כדוריד', team: true},
                    {name: 'טניס שולחן', team: true},
                    {name: 'החלקה אמנותית'},
                    {name: 'כדור ים'},
                    {name: 'טריאתלון'},
                    {name: 'התעמלות'},
        ...
        mounted: function () {
            ...
            Vue.http.get(url).then(function(resp) {
                ...
                utils.readServerCache('pele-team-registration', true, function(err, value) {
        ...
        methods: {
            ...
            newTeam: function() {
                var comp = this;
                Dialog.open("registration/project/pele-team-dialog", {facilityTypes: comp.facilityTypes, sports: comp.sports},
                    function (err, result) {
                        if (result == null) {
                            return;
                        }
            ...
            editTeam: function(){
                ...
                Dialog.open("registration/project/pele-team-dialog", {facilityTypes: comp.facilityTypes, sports: comp.sports, team: team},
                    function (err, result) {
                        if (result == null) {
                            return;
                        }
            ...
            next: function () {
                // go to next page
                Vue.http.put('/api/v2/registration/project/3/status/2', {});
                this.$emit('next')
            },
    ...
    return RegistrationProjectPeleTeamsComponent;
});