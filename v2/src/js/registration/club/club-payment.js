define(["templates/registration", "services/access", "services/products", "dialog", "consts", "utils"], function (
                            ...
                            for (var i = 0; i < resp.body.length; i++) {
                                ...
                                if (team.sport) {
                                    ...
                                    if (team.competition) {
                                        ...
                                        /* removed - should already be marked by API
                                        if (team.sport.isOnePaymentPerCategory) {
                                            if (team.sport.name.indexOf('כדורסל') >= 0) {
    ...
    var RegistrationClubPaymentComponent = Vue.extend({
        ...
        mounted: function () {
            ...
            setTimeout(function() {
                comp.payments.forEach(function(payment) {
                    payment.method = '1'; //default
                });
                console.log(comp.payments);
            }, 2000);
        },
        computed: {
            canPay: function () {
                ...
                for (var i = 0; i < this.payments.length; i++) {
                    ...
                    if (payment.payerName == null || payment.payerName.length === 0) {
                        return false;
                    }
                    ...
                    .then(
                        function (resp) {
                            ...
                            var dialogMessage =
                                ...
                                "8. לשאלות בנוגע להרשמה ניתן לפנות אל מנהל/ת המחלקה לאירועי ספורט ";
                            if (comp.coordinator != null) {
                                dialogMessage += " של מחוז " + comp.coordinator.areaName + "<br/>" +
                                    "&nbsp;&nbsp; במייל " + comp.coordinator.mail + " <br/>&nbsp;&nbsp; בנייד " + comp.coordinator.phone;
                            } else {
                                dialogMessage += 'במחוזך';
                            }
                            Dialog.open("general/message-box",
                                {
                                    wide: true,
